...
class AntColony(object):
    ...
    def __init__(self, distances, n_ants, n_best, n_iterations, decay, alpha=1, beta=1):
        ...
        self.pheromone = np.ones(self.distances.shape) / len(distances) # initializing all to 1/d
    ...
    def run(self):
        ...
        animator = ani.FuncAnimation(self.fig, self.buildmebarchart, interval = 100)
        plt.show()        
    ...
    def spread_pheronome(self, all_paths, n_best, shortest_path):
        ...
        for path, dist in sorted_paths[:n_best]: #only the n best ants spread pheromones - darwin
            for move in path:
    ...
    def gen_path(self, start):
        ...
        for i in range(len(self.distances) - 1):
            move = self.pick_move(self.pheromone[prev], self.distances[prev], visited) #pick random node from unvisited nodes
    ...
    def pick_move(self, pheromone, dist, visited):
        ...
        row = pheromone ** self.alpha * (( 1.0 / dist) ** self.beta) # adjust pheromone level acc to weight

        norm_row = row / row.sum()
        move = np_choice(self.all_inds, 1, p=norm_row)[0] #pick 1 random index for move
        return move
